...
function mostrarCandidatos() {
  ...
  Swal.fire({
    ...
    if (res.isConfirmed) {
      ...
      // Construir lista de votos actualizada
      const resumenVotos = candidatos.map((c, i) => {
        return `<li><strong>${c.nombre} ${c.apellido}</strong>: ${votos[i]} voto(s)</li>`;
      }).join("");
      ...
      Swal.fire({
        ...
        html: `
          ...
          <ul style="text-align: left;">${resumenVotos}</ul>
        `,
        icon: "success"
      });
    }
